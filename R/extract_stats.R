...
#' intercepts (when `subject_ids` is provided).
#'
...
if (!is.null(subject_ids)) {
  ...
  # Fit mixed-effects model with subject as a random effect
  model <- stats::lm(standard_scores ~ measurement_values_centered, data = data)

  # Extract fixed-effect t-statistic for `measurement_values`
  model_summary <- summary(model)

  if (!"measurement_values_centered" %in% rownames(model_summary$coefficients)) {
    stop(sprintf("Retrodiction model failed (likely due to pathological measurement values). Please check your data."))
  }

  t_statistic <- model_summary$coefficients["measurement_values_centered", "t value"]
